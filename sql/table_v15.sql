-- ======================================================
-- 智慧养老平台数据库表结构（V15）
-- 在 V14 基础上新增 IoT 设备与家庭级硬件 SOS，对齐 iot_device_minimal_backend.sql：
--   1) 新增 families / family_members / iot_devices
--   2) emergency_alerts 扩展 family_id、source、device_id、area、hardware_message
--   3) location_logs / geofences 经纬度 DECIMAL(10,7)，支持小数点后 5 位以上精度
-- V14 已有：兴趣社群群聊 image、direct_message_clear、interest_community_chat_clear 等
-- 知识库数据请用后端导入任务或 CSV 自行导入，勿将全量 INSERT 提交进 Git
-- 新建库：先执行本文件，再执行 initial_v15.sql（如需）
-- 自 V14 升级：可仅执行文末「V14 → V15 迁移」段（CREATE + ALTER）
-- ======================================================
...
CREATE TABLE emergency_alerts (
  ...
  remark VARCHAR(500) DEFAULT NULL COMMENT '备注',
  family_id BIGINT DEFAULT NULL COMMENT '家庭ID，硬件SOS优先绑定家庭',
  source VARCHAR(32) DEFAULT NULL COMMENT '告警来源：APP/HARDWARE_DEVICE/RULE_ENGINE',
  device_id VARCHAR(64) DEFAULT NULL COMMENT '硬件设备编号',
  area VARCHAR(100) DEFAULT NULL COMMENT '硬件触发区域',
  hardware_message VARCHAR(255) DEFAULT NULL COMMENT '硬件SOS消息',
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  KEY idx_elder_profile_id (elder_profile_id),
  KEY idx_status (status),
  KEY idx_trigger_time (trigger_time),
  KEY idx_revoke_deadline (revoke_deadline),
  KEY idx_emergency_alerts_family_id (family_id),
  KEY idx_emergency_alerts_source (source),
  KEY idx_emergency_alerts_device_id (device_id)
...
--   ADD COLUMN family_id BIGINT DEFAULT NULL COMMENT '家庭ID，硬件SOS优先绑定家庭' AFTER remark,
--   ADD COLUMN source VARCHAR(32) DEFAULT NULL COMMENT '告警来源：APP/HARDWARE_DEVICE/RULE_ENGINE' AFTER family_id,
--   ADD COLUMN device_id VARCHAR(64) DEFAULT NULL COMMENT '硬件设备编号' AFTER source,
--   ADD COLUMN area VARCHAR(100) DEFAULT NULL COMMENT '硬件触发区域' AFTER device_id,
--   ADD COLUMN hardware_message VARCHAR(255) DEFAULT NULL COMMENT '硬件SOS消息' AFTER area;
--
-- ALTER TABLE emergency_alerts
...
--
-- -- 经纬度精度（Hibernate 未标注 scale 时可能落成 DECIMAL(19,2)，按需执行）
-- ALTER TABLE location_logs
--   MODIFY COLUMN latitude DECIMAL(10, 7) NOT NULL COMMENT '纬度',
--   MODIFY COLUMN longitude DECIMAL(10, 7) NOT NULL COMMENT '经度';
-- ALTER TABLE geofences
--   MODIFY COLUMN center_latitude DECIMAL(10, 7) NOT NULL COMMENT '中心纬度',
--   MODIFY COLUMN center_longitude DECIMAL(10, 7) NOT NULL COMMENT '中心经度';
