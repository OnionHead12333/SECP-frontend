...
-- 新建库：先执行本文件，再执行合并后的 initial.sql（如需）
...
DROP TABLE IF EXISTS robot_entertainment_task;
DROP TABLE IF EXISTS robot_navigation_task;
DROP TABLE IF EXISTS robot_command_log;
DROP TABLE IF EXISTS robot_event;
DROP TABLE IF EXISTS robot_map_marker;
DROP TABLE IF EXISTS robot_status;
DROP TABLE IF EXISTS robot_map;
DROP TABLE IF EXISTS robot_music_library;
...
CREATE TABLE users (
  ...
  role ENUM('elder','child','staff') NOT NULL COMMENT '角色：elder老人/child子女/staff员工',
...
-- ======================================================
-- V15 Robot / ROS2 / Inspection / Entertainment tables
-- ======================================================

-- 2. Robot device information.
CREATE TABLE IF NOT EXISTS robot_device (
  id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '小车设备ID',
  name VARCHAR(50) NOT NULL COMMENT '小车名称',
  ip VARCHAR(50) DEFAULT NULL COMMENT '小车或Jetson IP',
  control_port INT DEFAULT 6000 COMMENT '小车原控制服务端口',
  gateway_url VARCHAR(255) DEFAULT NULL COMMENT 'ROS2网关地址',
  video_url VARCHAR(255) DEFAULT NULL COMMENT '视频流或视频页面地址',
  status ENUM('online','offline') DEFAULT 'offline' COMMENT '设备状态',
  remark VARCHAR(255) DEFAULT NULL COMMENT '备注',
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  KEY idx_status (status)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='智能小车设备表';

-- 3. Latest robot status. Usually one row per robot.
CREATE TABLE IF NOT EXISTS robot_status (
  id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '状态ID',
  robot_id BIGINT NOT NULL COMMENT '小车设备ID',
  online TINYINT(1) DEFAULT 0 COMMENT '是否在线',
  battery INT DEFAULT NULL COMMENT '电量百分比',
  control_status ENUM('idle','moving','stopped') DEFAULT 'idle' COMMENT '控制状态',
  nav_status ENUM('idle','running','arrived','failed','paused') DEFAULT 'idle' COMMENT '导航状态',
  obstacle_status ENUM('safe','obstacle') DEFAULT 'safe' COMMENT '避障状态',
  fall_status ENUM('normal','fall') DEFAULT 'normal' COMMENT '跌倒检测状态',
  current_x DOUBLE DEFAULT NULL COMMENT '当前地图X坐标',
  current_y DOUBLE DEFAULT NULL COMMENT '当前地图Y坐标',
  current_yaw DOUBLE DEFAULT NULL COMMENT '当前朝向',
  last_message VARCHAR(255) DEFAULT NULL COMMENT '最近状态说明',
  updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  UNIQUE KEY uk_robot_id (robot_id),
  KEY idx_nav_status (nav_status),
  KEY idx_obstacle_status (obstacle_status),
  KEY idx_fall_status (fall_status)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='智能小车当前状态表';

-- 4. SLAM map or demo map.
CREATE TABLE IF NOT EXISTS robot_map (
  ...
  image_url VARCHAR(255) NOT NULL COMMENT '地图图片地址',
  width INT DEFAULT NULL COMMENT '地图图片宽度',
  height INT DEFAULT NULL COMMENT '地图图片高度',
  resolution DOUBLE DEFAULT NULL COMMENT '地图分辨率，可选',
  origin_x DOUBLE DEFAULT NULL COMMENT '地图原点X，可选',
  origin_y DOUBLE DEFAULT NULL COMMENT '地图原点Y，可选',
  is_active TINYINT(1) DEFAULT 1 COMMENT '是否当前使用',
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  KEY idx_active (is_active)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='智能小车地图表';

-- 5. Markers displayed on inspection map.
CREATE TABLE IF NOT EXISTS robot_map_marker (
  id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '地图标记ID',
  map_id BIGINT NOT NULL COMMENT '地图ID',
  robot_id BIGINT DEFAULT NULL COMMENT '小车设备ID',
  elder_profile_id BIGINT DEFAULT NULL COMMENT '关联老人档案ID，可为空',
  marker_type ENUM('fall','crack','obstacle','robot','target','manual') NOT NULL COMMENT '标记类型',
  level ENUM('info','warning','danger') DEFAULT 'info' COMMENT '等级',
  title VARCHAR(100) NOT NULL COMMENT '标题',
  description VARCHAR(500) DEFAULT NULL COMMENT '详情',
  location_name VARCHAR(100) DEFAULT NULL COMMENT '位置名称',
  location_x DOUBLE NOT NULL COMMENT '地图X坐标',
  location_y DOUBLE NOT NULL COMMENT '地图Y坐标',
  image_url VARCHAR(255) DEFAULT NULL COMMENT '截图地址',
  event_time DATETIME DEFAULT NULL COMMENT '事件发生时间，可由网关上报',
  elder_name VARCHAR(50) DEFAULT NULL COMMENT '识别到的老人姓名，可为空',
  identity_source ENUM('current_frame','recent_identity','unknown') DEFAULT NULL COMMENT '身份来源',
  identity_confidence DOUBLE DEFAULT NULL COMMENT '身份识别置信度',
  notified_child TINYINT(1) DEFAULT 0 COMMENT '是否已通知子女',
  navigation_status ENUM('idle','running','arrived','failed','paused') DEFAULT NULL COMMENT '导航状态',
  obstacle_status ENUM('safe','obstacle','unknown') DEFAULT NULL COMMENT '避障状态',
  source VARCHAR(50) DEFAULT 'mock' COMMENT '来源，如 mock/yolo/crack_detect/slam/gateway/staff',
  status ENUM('unhandled','handled','active','ignored') DEFAULT 'unhandled' COMMENT '处理状态',
  created_by BIGINT DEFAULT NULL COMMENT '创建人用户ID',
  handled_by BIGINT DEFAULT NULL COMMENT '处理人用户ID',
  handled_by_name VARCHAR(50) DEFAULT NULL COMMENT '处理人姓名，本地演示可直接存员工A',
  handle_remark VARCHAR(500) DEFAULT NULL COMMENT '处理备注',
  handled_at DATETIME DEFAULT NULL COMMENT '处理时间',
  payload_json TEXT DEFAULT NULL COMMENT '原始上报JSON，便于调试网关字段',
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  KEY idx_map_type (map_id, marker_type),
  KEY idx_marker_elder_type (elder_profile_id, marker_type),
  KEY idx_marker_status (status),
  KEY idx_marker_event_time (event_time),
  KEY idx_created_at (created_at)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='智能小车巡检地图标记表';

-- 6. Robot and AI events.
CREATE TABLE IF NOT EXISTS robot_event (
  id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '事件ID',
  robot_id BIGINT DEFAULT NULL COMMENT '小车设备ID',
  elder_profile_id BIGINT DEFAULT NULL COMMENT '关联老人档案ID，可为空',
  event_type ENUM('fall','crack','obstacle','navigation','system','entertainment') NOT NULL COMMENT '事件类型',
  level ENUM('info','warning','danger') DEFAULT 'info' COMMENT '事件等级',
  title VARCHAR(100) NOT NULL COMMENT '标题',
  message VARCHAR(500) DEFAULT NULL COMMENT '事件说明',
  location_x DOUBLE DEFAULT NULL COMMENT '地图X坐标',
  location_y DOUBLE DEFAULT NULL COMMENT '地图Y坐标',
  image_url VARCHAR(255) DEFAULT NULL COMMENT '截图地址',
  status ENUM('unhandled','handled','ignored') DEFAULT 'unhandled' COMMENT '处理状态',
  handled_by BIGINT DEFAULT NULL COMMENT '处理人用户ID',
  handled_at DATETIME DEFAULT NULL COMMENT '处理时间',
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  ...
  KEY idx_event_elder (elder_profile_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='智能小车事件告警表';

-- 7. Command log for control, navigation, music and dance.
CREATE TABLE IF NOT EXISTS robot_command_log (
  id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '命令日志ID',
  robot_id BIGINT DEFAULT NULL COMMENT '小车设备ID',
  user_id BIGINT DEFAULT NULL COMMENT '发起用户ID',
  command_type ENUM('control','navigation','music','dance','system') NOT NULL COMMENT '命令类型',
  command VARCHAR(100) NOT NULL COMMENT '命令名称',
  request_json TEXT DEFAULT NULL COMMENT '请求参数JSON',
  status ENUM('pending','sent','success','failed') DEFAULT 'pending' COMMENT '命令状态',
  response_message VARCHAR(500) DEFAULT NULL COMMENT '响应说明',
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  KEY idx_command_type (command_type),
  KEY idx_command_status (status),
  KEY idx_command_created (created_at)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='智能小车命令记录表';

-- 8. Navigation task.
CREATE TABLE IF NOT EXISTS robot_navigation_task (
  id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '导航任务ID',
  robot_id BIGINT DEFAULT NULL COMMENT '小车设备ID',
  creator_id BIGINT DEFAULT NULL COMMENT '发起用户ID',
  map_id BIGINT DEFAULT NULL COMMENT '地图ID',
  target_name VARCHAR(100) NOT NULL COMMENT '目标点名称',
  target_x DOUBLE NOT NULL COMMENT '目标X坐标',
  target_y DOUBLE NOT NULL COMMENT '目标Y坐标',
  status ENUM('pending','running','arrived','failed','cancelled') DEFAULT 'pending' COMMENT '任务状态',
  started_at DATETIME DEFAULT NULL COMMENT '开始时间',
  finished_at DATETIME DEFAULT NULL COMMENT '结束时间',
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  KEY idx_nav_status (status),
  KEY idx_nav_created (created_at)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='智能小车导航任务表';

-- 9. Music library for elder/staff entertainment pages.
CREATE TABLE IF NOT EXISTS robot_music_library (
  ...
  artist VARCHAR(100) DEFAULT NULL COMMENT '歌手或来源',
  duration_seconds INT DEFAULT NULL COMMENT '时长秒数',
  suitable_scene VARCHAR(100) DEFAULT NULL COMMENT '适合场景，如 exercise/dance/relax',
  enabled TINYINT(1) DEFAULT 1 COMMENT '是否启用',
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  KEY idx_music_enabled (enabled),
  KEY idx_music_scene (suitable_scene)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='智能小车音乐库';

-- 10. Entertainment task.
CREATE TABLE IF NOT EXISTS robot_entertainment_task (
  id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '娱乐任务ID',
  robot_id BIGINT DEFAULT NULL COMMENT '小车设备ID',
  user_id BIGINT DEFAULT NULL COMMENT '发起用户ID',
  music_id BIGINT DEFAULT NULL COMMENT '音乐ID，可为空',
  task_type ENUM('music','dance','music_dance') NOT NULL COMMENT '任务类型',
  music_name VARCHAR(100) DEFAULT NULL COMMENT '音乐名称',
  music_url VARCHAR(255) DEFAULT NULL COMMENT '音乐地址',
  dance_mode VARCHAR(50) DEFAULT NULL COMMENT '表演模式',
  request_json TEXT DEFAULT NULL COMMENT '请求参数JSON',
  status ENUM('pending','running','completed','failed','cancelled') DEFAULT 'pending' COMMENT '任务状态',
  response_message VARCHAR(500) DEFAULT NULL COMMENT '执行响应说明',
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  started_at DATETIME DEFAULT NULL COMMENT '开始时间',
  finished_at DATETIME DEFAULT NULL COMMENT '结束时间',
  updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  KEY idx_entertainment_status (status),
  KEY idx_entertainment_type (task_type),
  KEY idx_entertainment_music (music_id),
  KEY idx_entertainment_created (created_at)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='智能小车娱乐任务表';
