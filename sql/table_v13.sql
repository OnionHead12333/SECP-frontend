...
-- 智慧养老平台数据库表结构（V13）
-- 在 V12 基础上新增兴趣社群模块：
--   interest_communities、community_demo_peer_profiles、interest_community_memberships、
--   interest_community_messages、community_peer_seed_log、elder_friends、
--   direct_message_threads、direct_messages
-- 覆盖前端：兴趣社群 Tab、群聊语音/文字、好友、私聊、点头像加好友、相册头像（users.avatar_url）
-- 知识库数据请用后端导入任务或 CSV 自行导入，勿将全量 INSERT 提交进 Git
-- 新建库：先执行本文件，再执行 initial_v13.sql（如需）
-- ======================================================
...
-- 16.3 群聊消息（前端 CommunityDemoRepository / InterestCommunityVoiceMessage）
CREATE TABLE interest_community_messages (
  ...
  message_kind ENUM('voice','text') NOT NULL DEFAULT 'text' COMMENT '消息类型',
  text_content TEXT DEFAULT NULL COMMENT '文字内容',
  audio_url VARCHAR(512) DEFAULT NULL COMMENT '语音文件 URL（前端 audioPath）',
...
CREATE TABLE direct_messages (
  ...
  sender_role ENUM('elder','child') NOT NULL DEFAULT 'elder' COMMENT '发送角色',
  message_kind ENUM('voice','text') NOT NULL DEFAULT 'voice' COMMENT '消息类型',
  text_content TEXT DEFAULT NULL COMMENT '文字内容',
