...
import 'child_ai_consultation_history_page.dart';

class ChildAiMedicalAssistantPage extends StatefulWidget {
  const ChildAiMedicalAssistantPage({
    super.key,
  ...
  @override
  State<ChildAiMedicalAssistantPage> createState() =>
      _ChildAiMedicalAssistantPageState();
}

class _ChildAiMedicalAssistantPageState extends State<ChildAiMedicalAssistantPage> {
  ...
  Future<void> _submit([String? quick]) async {
    ...
    final elderlyId = widget.elderProfileId;
    if (elderlyId <= 0) {
      _toast('当前没有获取到绑定老人信息，请先选择老人。');
      return;
    }
    if (text.isEmpty) {
      _toast('请先输入不舒服的症状。');
    ...
    try {
      final res = await ChildAiMedicalAssistantApi.createConsultation(
        elderlyId: elderlyId,
        inputText: text,
        inputType: 'text',
      );
  ...
  Future<void> _followUp() async {
    ...
    setState(() => _loading = true);
    try {
      final res = await ChildAiMedicalAssistantApi.sendMessage(
        id: id,
        messageContent: text,
  ...
  Future<void> _notifyFamily() async {
    ...
    setState(() => _loading = true);
    try {
      final res = await ChildAiMedicalAssistantApi.notifyFamily(id);
  ...
  Future<void> _submitFeedback(
    ...
    try {
      await ChildAiMedicalAssistantApi.submitFeedback(
        id,
          ...
          Align(
            ...
            child: OutlinedButton.icon(
              onPressed: () => Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (_) => ChildAiConsultationHistoryPage(
                      elderlyId: widget.elderProfileId),
                ),
              ),
