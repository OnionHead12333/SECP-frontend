...
final class ChildAiMedicalAssistantApi {
  ChildAiMedicalAssistantApi._();

  static const String _base = '/v1/child/ai/consultations';
  ...
  static Future<AiNotifyFamilyResponse> notifyFamily(int id) async {
    final res =
        await ApiClient.dio.post<Map<String, dynamic>>('$_base/$id/notify-family');
    return _parseData(res.data, AiNotifyFamilyResponse.fromJson);
  }

  static Future<void> submitFeedback(
    int id,
    AiFeedbackRequestModel request,
  ) async {
    final res = await ApiClient.dio.post<Map<String, dynamic>>(
      '$_base/$id/feedback',
      data: request.toJson(),
    ...
    if (!api.isSuccess) {
      throw Exception(api.displayMessage);
    }
  }
  ...
  static Future<List<AiConsultationHistoryItem>> getConsultationHistory({
    ...
    return items
        .whereType<Map>()
        .map(
          (item) => AiConsultationHistoryItem.fromJson(
  ...
  static Future<List<RecommendedDepartment>> getRecommendedDepartments(
    int id,
  ) async {
    final res = await ApiClient.dio.get<Map<String, dynamic>>(
      '$_base/$id/recommended-departments',
    );
    final body = res.data;
    if (body == null) {
      throw Exception('服务返回为空，请稍后重试。');
    ...
    return (api.data as List)
        .whereType<Map>()
        .map(
          (item) => RecommendedDepartment.fromJson(
            Map<String, dynamic>.from(item),
          ),
        )
        .toList();
