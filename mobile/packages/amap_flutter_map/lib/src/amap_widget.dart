...
class _MapState extends State<AMapWidget> {
  ...
  void _updateMarkers() async {
    final Set<Marker> previous = _markers.values.toSet();
    final MarkerUpdates updates = MarkerUpdates.from(previous, widget.markers);
    _markers = keyByMarkerId(widget.markers);
    if (updates.markersToAdd!.isEmpty &&
        updates.markerIdsToRemove!.isEmpty &&
        updates.markersToChange!.isEmpty) {
      return;
    }
    final AMapController controller = await _controller.future;
    // ignore: unawaited_futures
    controller._updateMarkers(updates);
  }

  void _updatePolylines() async {
    final Set<Polyline> previous = _polylines.values.toSet();
    final PolylineUpdates updates = PolylineUpdates.from(previous, widget.polylines);
    _polylines = keyByPolylineId(widget.polylines);
    if (updates.polylinesToAdd!.isEmpty &&
        updates.polylineIdsToRemove!.isEmpty &&
        updates.polylinesToChange!.isEmpty) {
      return;
    }
    final AMapController controller = await _controller.future;
  ...
  void _updatePolygons() async {
    final Set<Polygon> previous = _polygons.values.toSet();
    final PolygonUpdates updates = PolygonUpdates.from(previous, widget.polygons);
    _polygons = keyByPolygonId(widget.polygons);
    if (updates.polygonsToAdd!.isEmpty &&
        updates.polygonIdsToRemove!.isEmpty &&
        updates.polygonsToChange!.isEmpty) {
      return;
    }
    final AMapController controller = await _controller.future;
